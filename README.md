...
- Captures tab: list / delete stored capture groups, **Process** a group through the on-device preprocessing pipeline (which also derives that group's normalization parameters), **Pick** a processed group as the one whose parameters get staged with a model, and import a subject dataset (`.ssds`) exported by `backend/scripts/system/export_subject_data.py`. The `.ssds` file is a `somasafe.capture.CaptureDataset` protobuf (schema in `shared/dataset.proto`); imported windows are stored as ordinary samples — sequence number, on-device timestamps, a back-dated receive time, raw PPG/ACC, raw float32 features, label in the score field — indistinguishable from ESP captures, so they preprocess and train identically without streaming over BLE. The export can also model packet loss: a window may carry signal but no ML result (the phone recomputes features), a result but no signal, or be dropped entirely (a hole in the sequence numbers), so import handles each missing-data case just as a lossy live capture would.
- On-device preprocessing pipeline (capture module): replicate the firmware's 17-feature `extract_features` (via JDSP for the FFT) so windows whose ML result was lost still get a feature vector (stored without a score), and derive the wearer's z-score parameters from the group's own windows — per-feature over the raw feature vectors, plus one pair over the raw BVP stream — since no model normalizes its own input.
- On-device training (training module): pick a downloaded model, run a local epoch over a processed capture group (starting from the weights baked into the trainable artifact, or from a previous local epoch's `trained_weights.bin`), and write the trained weights to `trained_weights.bin` (with the training baseline in `base_weights.bin`) — from where the upload actions submit the delta (`trained − global`) as the federated update ("Upload & quantize" returns a personalized signed int8 artifact; "Submit only" sends the update and nothing comes back). Reached from the model detail screen ("Train on capture…"). Every window is z-scored on the phone with the capture group's own signal parameters before it reaches the `train` signature — the models take already-normalized input (see [`shared/docs/model-types.md`](../shared/docs/model-types.md)), and the parameters are the wearer's own, never the server's. The epoch runs batch by batch on the phone (the native `train` call is one gradient step), so the screen follows it on a progress bar and the run can be cancelled between steps, writing nothing. When it finishes it reports the run's metrics: the mean per-window reconstruction error over the same 20 randomly drawn batches, measured with the `eval` signature before and after the epoch (that is plain MSE, not the `train` loss, which carries an extra first-difference term), the mean and last-batch train loss, how many windows were trained, dropped for missing signal or left over by the partial batch, the L2 norm and largest component of the `trained − base` update that an upload would submit, and the wall clock of each phase. The metrics live on the screen only — nothing extra is written to disk.
- Device payload assembly: the app frames the signed quantized model (`sig_len ‖ sig ‖ norm_len ‖ norm params ‖ contract_version ‖ tflite`, per the BLE interface version — see [`shared/docs/model-signing.md`](../shared/docs/model-signing.md)) before staging it onto the ESP32, supplying the wearer's own normalization parameters from the capture store; the server signs `contract_version ‖ tflite` only.
...
app/src/main/
  kotlin/app/somasafe/
    ...
    capture/                 Capture storage + on-device preprocessing (Room db, 17-feature extraction
                             mirroring the firmware, per-wearer normalization params, .ssds dataset
                             import) — no device interaction
    training/                On-device LiteRT training: JNI wrapper + local-epoch trainer over a
                             processed capture group, with per-batch progress and end-of-run metrics
...
models/
  <key>/
    ...
    quantized.json       # signed fields delivered with it: signature, contract version, architecture version (QUANTIZED_META_FILENAME)
```
...
Which paths a model accepts is set per model by its `submission_type` (from `/model/list`, see [`shared/docs/submission-type.md`](../shared/docs/submission-type.md)): a `quantize` model accepts both, a `raw` model accepts only "Submit only" (the quantize path returns `404` for it), and a `secure` model accepts neither — its masked updates go through the sealed-round endpoints, which the app has no client for. The app hides "Upload & quantize" for `raw` models and both actions for `secure` ones. Server-side, aggregation adds the averaged delta back onto the global weights, so a client that trained nothing submits ≈0 and moves the global model not at all.

In both download paths the response body is the bare int8 `.tflite` and the signed fields travel in headers (`X-Model-Signature`, base64, and `X-Contract-Version`); they are stored as `quantized.tflite` + `quantized.json`. Normalization is not among them — it is per wearer, so the server has nothing to say about it. The app assembles the device payload at staging time (`bluetooth/domain/ModelPayload.kt`, per [`shared/docs/model-signing.md`](../shared/docs/model-signing.md)) from those fields plus the picked capture group's parameters — the server signature covers `contract_version ‖ tflite` only, is transport-independent, and is verified by the firmware.
...
| GET | `/model/download/quantized/<key>` | Global int8 `.tflite`; version headers plus the signed fields (`X-Model-Signature` / `X-Contract-Version`) |
...
1. Pick a downloaded model and **Load** it — the payload (the signed model plus the wearer's normalization parameters, from the picked capture group) is staged into the device's client buffer and marked READY; the model's input/output tensor sizes are read via the LiteRT bridge to know the result layout.
...
Each group also stores the normalization parameters preprocessing derived from it (see below). Sequence numbers are unique only within a group — the firmware resets them on reboot — so matching is always scoped to a single group. Samples may be lost mid-group without affecting the rest. Each sample stores the raw PPG/ACC floats, the on-device start/end timestamps, the receive time, and (when present) the raw float32 features and the int8 model score. The Room database lives at `capture.db`.
...
- **Normalization parameters** — the same pass derives the wearer's z-score parameters from that group's own windows and stores them on the group: per-feature mean/std over every raw feature vector (17 + 17, the block a staged model carries) and one mean/std over the raw BVP stream (what on-device training normalizes its windows with). Both are computed as `mean`, `std + 1e-6`, matching `norm_stats` in `backend/ml/sources/`.

Samples are stored **un-normalized** everywhere — raw signal, raw features, exactly as the firmware reports them — and whoever feeds a model z-scores them first. No model normalizes its own input any more and normalization is **per wearer**, so nothing about it travels with a model: the phone derives it from its own capture store (see [`shared/docs/model-types.md`](../shared/docs/model-types.md)).

### Which capture's parameters get staged

Preprocessing computes parameters per capture group, so one group has to be nominated as the wearer's reference. Once a group has been processed its **Process** button becomes **Pick**; picking it greys the button out, marks the card "normalization parameters selected for staging" and clears any previous pick (only one group is ever picked). With nothing picked, staging falls back to the most recently started processed group; with no processed group at all the model load is rejected and the connected-device screen shows why. On-device training does not use the pick — it always normalizes with the parameters of the group it is training on.

The `.ssds` import is a `somasafe.capture.CaptureDataset` protobuf (`shared/dataset.proto`, generated into the build dir by the protobuf Gradle plugin — see [Generated code](#generated-code)). It carries windows only — an imported group derives its normalization parameters through **Process** like any recorded one. Each window carries its sequence number and, alongside any signal data, faked device timestamps (a contiguous 8 s grid from a random boot offset). A window may legitimately arrive with signal but no features, features but no signal (no device timestamps, exactly like a result-only row), or be absent entirely (a gap in the sequence numbers). The receive time is stamped by the phone at import, back-dated per sequence step so dropped windows widen the gap.
...
2. Reads the train signature via `describe()` for its batch size. The signature takes a single input, `signal` (`[batch, 512, 1]`).
3. Assembles windows from a capture group — those carrying a 512-sample PPG window — and z-scores each one with that group's stored signal parameters, since the models take already-normalized input. The normalized window is the whole model input. The score/label is unused (the autoencoder is self-supervised), and so is ACC: the autoencoder does not take it as an input channel, and it reaches the models only through the 17-d feature vector. Windows are order-independent, so it trains whichever full batches exist and drops the remainder.
...
It requires the model to be downloaded and the capture group to have been processed (no parameters, no training); `TrainingScreen` gates on both.
