...
models/
  <key>/
    ...
    weights.json       # locally trained weights, written only by on-device training (WEIGHTS_FILENAME)
...
The backend bakes the current global weights into every trainable `.tflite` it serves, so downloading the model *is* downloading the weights — there is no separate weights pull. `downloadTrainable` records the snapshot the artifact carries (the `X-Weights-ID` / `X-Weights-Timestamp` / `X-Model-Fingerprint` / `X-Model-Version` headers) in `meta.json`; when the version or fingerprint moved relative to what was stored, the local federated state (`weights.json`, quantized artifact) is deleted first, since it belongs to the superseded version.

`weights.json` holds only a **locally trained update**: on-device training writes the trained parameters there together with the base `weights_id` from `meta.json`. `weightsStatus` (in `Weights.kt`) reports it `MISSING` (not trained yet — not an error), `OUTDATED` (the trainable was re-downloaded with a newer snapshot after training) or `CURRENT`. Whether the *model* is up to date is a separate check on the download screen: `meta.json`'s `version`/`fingerprint`/`weights_version` against the live `/model/list` entry.

### Quantized model and federated uploads

The device runs int8 inference, so it is the quantized model that gets staged during capture. There are two ways to obtain it, matching the backend's two submission architectures:

- **Download quantized** — plain `GET /model/download/quantized/<key>`: the current global int8 artifact. No training required.
- **Upload & quantize** — `uploadAndQuantize` POSTs the trained `weights.json` parameters as a raw little-endian float32 body to `POST /model/quantize/submit/<key>/<weights_id>` (the base `weights_id` rides the URL and pins the snapshot the update was trained against; that submission is the federated update). The backend enqueues a job and replies `202 {"job_id"}`; the app polls `GET /model/quantize/result/<job_id>` (`202` pending, `200` done, `422` failed) and stores the personalized int8 result.
- **Submit only** — `submitOnly` POSTs the same body to `POST /model/submit/<key>/<weights_id>`; the update feeds aggregation and nothing comes back (validation is server-side and silent).
...
Both the download screen and the model detail screen (both under the Backend tab) expose the three actions; the upload buttons stay disabled until on-device training has produced `weights.json`. Weight handling lives in `backend/data` and has no dependency on the LiteRT code; `quantStatus` (also in `Weights.kt`) flags the quantized artifact as stale when `weights.json` is newer than it, and missing when the signed fields (`quantized.json`) are absent.
...
| POST | `/model/quantize/submit/<key>/<weights_id>` | raw LE float32 body → `202` `{"job_id", "status_url"}` (federated update + quantize job) |
| GET | `/model/quantize/result/<job_id>` | `202` while pending/running, `200` int8 `.tflite` + signed headers when done, `422` on failure |
| POST | `/model/submit/<key>/<weights_id>` | raw LE float32 body → `202` `{"submission_id"}` (submit-only federated update) |
...
1. Loads the trainable LiteRT model (its baked-in weights are the global snapshot) and, when a previous epoch left a `weights.json`, restores those on top.
...
4. Writes the trained weights to `weights.json` tagged with the base `weights_id` from `meta.json`, which marks the quantized artifact outdated. The "Upload & quantize" / "Submit only" actions then submit them — that submission is the federated update.
