...
/**
 * One reassembled PPG window. [ppg] is the raw little endian float32 samples
 * (64 Hz). [deviceStartMs] and [deviceEndMs] are device-uptime timestamps.
 * [sequenceN] is the service-layer slice sequence number used to match against
 * ML results; it is NOT unique across device resets.
 */
 ...
 *   body:   sliceSeconds * ppgPerSec float32 samples
 *   tail:   end_ms(u32)
 */
class PpgService(private val connection: BleConnection) {
    ...
    private fun parse(payload: ByteArray): PpgSample? {
        ...
        val expected = HEADER_LEN + sliceSeconds * ppgBytesPerSec + TAIL_LEN
        if (payload.size != expected) {
            Log.w(TAG, "PPG payload length ${payload.size}, expected $expected for $sliceSeconds s")
            return null
        }

        val ppg = payload.copyOfRange(HEADER_LEN, HEADER_LEN + sliceSeconds * ppgBytesPerSec)

        return PpgSample(sequenceN, sliceSeconds, startMs, endMs, ppg)
    }

    companion object {
        // Per-second sample count, mirroring firmware PPG_SAMPLE_RATE.
